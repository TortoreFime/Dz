...
export default function UserIndex({ orders }) {
  ...
  const change = (orderId) => {
    const updatedOrders = orderList.map((order) => {
      if (order.id === orderId) {
        const newState = order.cookState === "Готовится" ? "Приготовлен" : "Готовится";
  ...
  return (
    <div>
      <header style={{
        backgroundColor: "#d1d1d1ff", height: "40px", fontFamily: "sans-serif",
        ...
        <ul style={{ padding: 0 }}>
          {orderList.map(order => (
            <div key={order.id} style={{ display: 'flex', marginBottom: "20px", alignItems: "center" }}>
              <li style={{ listStyleType: "none", marginRight: "15px" }}>{order.id} заказ: {order.cookState}</li>
